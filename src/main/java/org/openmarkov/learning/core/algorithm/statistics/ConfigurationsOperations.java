...
public class ConfigurationsOperations {
	 ...
	 * @param interestVariables. <code>ArrayList</code> of <code>Variable</code>
	 * @param allVariables. <code>ArrayList</code> of <code>Variable</code> 
	...
	public static TablePotential getCPT(int[][] data, 
    	...
    	int numConditioningConfigurations = 
    		potential.values.length / numStatesConditioned;
    	for (int i = 0; i < numConditioningConfigurations; i++) {
    		...
    		for (int j = 0; j < numStatesConditioned; j++) {
    			potential.values[i * numStatesConditioned + j] = 
    				count.values[i * numStatesConditioned + j] / acc;
    		}    		
    	}
    	return potential;
	}
    ...
    public static TablePotential count(int[][] data, 
    	...
    	for (int i = 0; i < numInterestVariables; i++) {
    		interestVariablesPositions[i] = 
    			allVariables.indexOf(interestVariables.get(i));
    	}
    ...
    /** Generates a long <code>String</code> that contains:<ul>
     * <li>The list of the potential variables
     * <li>All the configurations. Each configuration contains:<ul>
     ...
     * @param confTitle. <code>String</code> corresponding to the column of the
     *   values of the configurations.
     * @param accuracy. <code>double</code>
    ...
    public static String getStringPotential(TablePotential potential, 
    	...
    	for (int i = 0; i < potential.values.length; i++) {
    		...
    		for (int j = 0; j < numVariables; j++) {
    			String stateName = variables.get(j)
    				.getStateName(configuration[j]);
    			out += stateName;
    		...
    		out += new Double(Math.round(potential.values[i] * 
    				Math.pow(10, accuracy)) / Math.pow(10, accuracy));
			out += EOL;
    	}
    	out += line + EOL;
    ...
    /** @param potential. <code>TablePotential</code>
     * @param variablesNumChars. <code>int[]</code>.
	...
	private static String getSeparationLine(TablePotential potential, 
    	...
    	for (int i = 0; i < numVariables; i++) {
        	for (int j = 0; j < variablesNumChars[i] + SEPARATION_STRING_LENGTH;
        			j++) {
        		line += MINUS_CHAR;
        	}    	
    	}
